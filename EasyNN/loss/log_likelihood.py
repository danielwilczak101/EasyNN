...
class LogLikelihood(Loss):
    ...
    Consider using the LogSoftMax activation with the NegativeLikelihood loss instead.
    """

    def __call__(self: LogLikelihood, x: Array1D, y: Array1D) -> float:
        # Check the inputs.
        ...
        # return -log(y).
        loss = np.asarray(likelihoods, dtype=float)
        return -np.log(loss, out=loss).mean()

    def backward(self: LogLikelihood, y: Array1D, y_pred: Array1D, model: EasyNN.model.abc.Model = None) -> Any:
        """
        Updates self.derivatives (e.g. via backpropagation) for optimizers (e.g. gradient descent).
        If the last layer is the SoftMax activation, then a special algorithm is used.
        ...
        use_y = isinstance(model.layers[-1], SoftMax)
        if model is None:
            model = self.model
        return model.backward(y_pred if use_y else self.dy(y, y_pred), y, use_y=use_y)

    def dy(self: LogLikelihood, y: Array1D, y_pred: Array1D) -> Array1D:
        # Check the inputs.
        ...
        # result = -1 / y_pred if y matches the sample, else 0.
        result = np.zeros_like(y_pred)
        # y = [arrays of indexes] e.g. y = [2, 1].
        if y.ndim == 1 < y_pred.ndim:
            result[np.arange(len(y)), y] = -1 / y_pred[np.arange(len(y)), y.astype(int)]
        # y = one index e.g. y = 2.
        elif y.ndim < y_pred.ndim:
            result[y] = -1 / y_pred[int(y)]
        # y = [arrays of 0 and 1] e.g. y = [0, 0, 1] or y = [[0, 0, 1], [0, 1, 0]].
        else:
            result[y != 0] = -1 / y_pred[y != 0]
        return result
